...
import (
	...
	"simple-terminal-chat/internals/room"
	"sync"
)
...
func main() {
	...
	for {
		...
		if err != nil {
			slog.Error("Connection accept error. Error:", err.Error())
		}

		acceptedClient := client.NewServerClient(conn)
		rootRoom.AddClient(acceptedClient)
